            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div className="space-y-8">
                    ...
                    {(health.components?.livenessState || health.components?.readinessState || health.components?.ping) && (
                        <div>
                            <h2 className="font-bold text-gray-900 mb-2 border-b border-gray-100 pb-1">SYSTEM RUNTIME</h2>
                            <div className="grid grid-cols-2 gap-4">
                                {health.components?.livenessState && (
                                    <>
                                        <span className="text-gray-500">Liveness</span>
                                        <span className={health.components.livenessState.status === 'UP' ? 'text-green-600' : 'text-red-600'}>{health.components.livenessState.status}</span>
                                    </>
                                )}
                                {health.components?.readinessState && (
                                    <>
                                        <span className="text-gray-500">Readiness</span>
                                        <span className={health.components.readinessState.status === 'UP' ? 'text-green-600' : 'text-red-600'}>{health.components.readinessState.status}</span>
                                    </>
                                )}
                                {health.components?.ping && (
                                    <>
                                        <span className="text-gray-500">Ping</span>
                                        <span className={health.components.ping.status === 'UP' ? 'text-green-600' : 'text-red-600'}>{health.components.ping.status}</span>
                                    </>
                                )}
                            </div>
                        </div>
